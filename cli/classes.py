...
class User:
    ...
    def greet(self) -> None:
        ...
        print("If you don't find what you are looking for, ", end="")
...
class Employee(User):
    def __init__(
        self, user_name: str, password: str = None, head_of: list[Employee] = None
    ) -> None:
    ...
    def authenticate(self, password: str) -> bool:
        """Returns True if the password provided by user
        is the same as password in system, otherwise True
        If User becomes authenticated parameter
        self.is_authenticated becomes True
        """
        match = password == self.__password
        if match:
            # NOTE: this is not specified!!
            # (but it makes sense, and the opposite doesn't)
    ...
    def greet(self) -> None:
        print(f"\nHello, {self._name}!")
        print("If you experience a problem with the system, ", end="")
...
class Warehouse:
    def __init__(self, id: int, stock: list[Item] = None) -> None:
        self.id = id
        self.stock = stock if stock is not None else []

    def __str__(self) -> str:
        return f"Warehouse ID: {self.id} ({self.stock} items in stock)"
    ...
    def search(self, search_term: str) -> list[Item]:
        return [
            ...
            if item.full_name().lower() == search_term.lower()
        ]
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, Item):
            return (
                ...
                and self.date_of_stock == other.date_of_stock
            )
        return NotImplemented

    def __hash__(self):
        # necessary for instances to behave sanely in dicts and sets.
        return hash((self.state, self.category, self.date_of_stock))

    def __str__(self) -> str:
        return f'''state="{self.state}",
                   category="{self.category}",
                   date_of_stock="{self.date_of_stock}"'''
